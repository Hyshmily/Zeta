...
6. **Collision guards.** Two instances produce equal degraded versions only when their Snowflake IDs collide: same 5ms TimeSource window + same worker id + same sequence. The worker id is randomized per JVM (InstanceIdGenerator node id XOR pid, 8-bit space — replaced the IP-last-octet scheme that collided systematically across K8s subnets), and the auto-configured generator randomizes the per-window sequence start (`randomSequence=true`), so equal IDs additionally require both instances to draw the same 4096-entry sequence start. Residual collision probability is negligible for realistic per-key write rates; a dropped broadcast self-heals via the next periodic sync cycle (ADR-0013, ADR-0024).
