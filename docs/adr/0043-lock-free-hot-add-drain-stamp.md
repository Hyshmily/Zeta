...
## Refinement (2026-08-13): thread-private last-key cache

A per-`Ceils` last-key cache (`lastKey` / `lastSlot`, owner-thread-only) short-circuits the repeated add of the SAME key instance — the common hot-key pattern — to one pointer test plus the existing stamp read and `getAndAdd`; the probe and the `equals` (the expensive step for long keys) are skipped. The cache is best-effort, never correctness: every claim (full path or `addSlow`) re-records it, so it can never point at a slot holding a DIFFERENT key (a claim is the only way a slot changes keys); the only stale states are drained-empty slots, which the pre-existing 0-return residual recovery resolves exactly, after which the cache is invalidated so the next add re-claims through the full path. `reset()` (pool reuse) and `drainInto` clear the cache for the same reason. The ADR-0043 protocol itself — stamp reads, post-check, reconcile, 0-return recovery — is untouched. Re-verified: full `WaveCounter*` suite green (72 tests, 0 failures).

## Refinement (2026-08-13): cache consulted before the routing beacon

The cache check moved OUT of `Ceils.add` into a pre-routing `tryFastAdd` called by `count()` BEFORE the beacon membership test, so a repeated-key op skips the beacon's two array loads and rehash (~3-5ns) as well. Correctness rests on the cache's construction: it can only reference a key that previously took the routed hot path (claims are the only way a slot changes keys, and claims only happen there), so a hit is a hot-key add by definition — routing is never consulted, exactness never depends on it. A key whose beacon evidence decayed out of the hot set keeps landing in the local map until the next drain (which invalidates the cache); its counts still merge into the shared table exactly — a stale hot route bounded by the next drain, in the same class as the documented beacon false positives. The stamp-odd (drain in flight) case returns `false` from `tryFastAdd` and falls through to the routed path's locked slow add, unchanged. Cold-only workloads trade the beacon test for one ThreadLocalMap miss-get (a wash); hot workloads save the beacon read per repeated-key op. The ADR-0043 protocol itself is untouched. Re-verified: full `WaveCounter*` suite green — 98 tests incl. the performance-tagged stress classes (`tryLockSkip_shouldNeverLoseHotCounts`, hot-racing exactness), 0 failures.

## Refinement (2026-08-14): flush-clock discharge removed

The per-writer flush-clock discharge (a sampled `lastFlushMillis` re-check with a `flushIntervalMs` fallback) was removed from the batch trigger entirely, together with the `lastFlushMillis` field and the `TIME_CHECK_MASK` sampling constant. Rationale: the clock-triggered discharge only moved local data into the shared table ~450ms earlier — delivery still waited for the next tide — so its only real value was covering the rare tide-`tryLock` collision (a writer mid-discharge when the tide drains), which the next tide's `tryDrainInto` covers anyway. The tide loop is the actual low-traffic fallback: every tide drains every registered writer's map (tryLock, dead-writer lock-free), so low-traffic residuals reach the shared table within one tide interval (~500ms) instead of ~50ms; hot-path exactness is untouched (only the fallback's detection latency widens). Removing it eliminates the sampled `TimeSource.monotonicMillis()` read from the hot path entirely — measured ~640ns/call on a virtualized host (the probe benchmark that motivated the change), ~25ns on bare metal. `flushIntervalMs` and `DEFAULT_FLUSH_INTERVAL_MS` are retained as deprecated API compatibility (accepted, ignored). Re-verified: full `WaveCounter*` suite green.
