...
**PromotionGovernor (floor).** Each promoted tide measures `renewal` = active hot slots whose key earned at least the threshold, divided by active slots (probed: healthy single-key set 1.0, one-drifter pair 0.5, fully drifted 0.0). Below the target (0.5) the set is distressed; the floor steps up (16, decaying ×0.98) ONLY while `blockedKeys > 0` (keys at the histogram boundary but below the floor — the only case a raise can help; the probe found unconditional raising starves low-traffic workloads). If distress survives 4 tides of raising, the floor retreats to its probe base in 8 budgeted strides (veto-return). Under health, the floor is released: saturation (active slots ≥ 90% of hotLimit) lowers it one step per tide, and a still-health audit (8 tides without movement) lowers it one step — the ratchet release. Clamped to [10, 256]. The histogram boundary stays scale-free; the governor only moves the noise filter.
...
2. **Idle cost drops**: after a stretch, empty cycles run at 2× the base interval; the quiescence window, decay sweep and scheduler wakeup are paid half as often. Worst-case detection of a burst arriving in the stretch is 2 × base interval (1,000ms at defaults).
