...
1. **共享广播交换机** — 复用现有的 `hotkey.broadcast.exchange`（即 HOT/COOL 决策所用的同一 exchange）。每个 Worker 的 `workerConfigQueue` 直接绑定到此交换机。
2. **每节点自动删除队列** — 每个 Worker 声明自己的 `hotkey.worker.config.<nodeId>` 队列并绑定到广播交换机。Worker 断开时，其队列自动删除。
3. **心跳头** — `WorkerBroadcaster` 将当前的 `confirmCount`、`coolCount`、`preCoolGraceCount` 和 `configTimestamp` 附加到发送到共享广播交换机的每条心跳消息上。
