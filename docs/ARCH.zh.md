...
│      decisionVersion,                 ← 保留原有值（尊重 Worker 决策）
│      hardTtlMs,  hardExpireAtMs,
│      softTtlMs,  softExpireAtMs,
│      keyState,                        ← 保留原有状态（HOT/COOL/NORMAL）
│      normalHardTtlMs, normalSoftTtlMs })
│
...
└─ HotKeyCache.get(cacheKey, reader, effectiveHardTtl, effectiveSoftTtl)
      ├─ ensureNotBlocked(key, "get")                          [RuleMatcher → BLOCK? 抛异常]
      ├─ isWhitelisted(key)?                                   [RuleMatcher → ALLOW_NO_REPORT? 跳过上报]
      ...
      │
      └─ 未命中 → ensureNotBlocked(key, "get")                 [RuleMatcher → BLOCK? 抛异常]
           └─ SingleFlight.execute(key, supplier)               [并发合并]
                ├─ supplier.get() → reader.get()                [用户提供的 L2/DB 读取]
                ├─ ensureNotBlocked(key, "get")                 [RuleMatcher 二次检查 → BLOCK? 抛异常 (TOCTOU 守卫)]
                ├─ hotKeyDetector.add(key, 1)
                ├─ hotKeyReporter.record(key)
                 └─ loadAndCache:
                      ├─ hotKeyDetector.add(cacheKey, 1)         [缓冲→刷新→堆]
                 ├─ hotKeyDetector.contains(cacheKey)             [堆检查]
                 │    ├─ true → CacheEntry(keyState=HOT, 热点 TTL)
                 │    │         [dataVersion=0L, decisionVersion=0L]
                 │    └─ false → CacheEntry(keyState=NORMAL, 普通 TTL)
                 │               [dataVersion=0L, decisionVersion=0L]
...
> [!NOTE] `hotKeyDetector.add(cacheKey, 1)` 将计数写入缓冲区而非直接更新堆。缓冲区每约 500ms 刷新一次，将累积计数批量合并到 HeavyKeeper 堆中。因此首次读取的 key 在刷新前不会出现在堆中，`contains()` 可能在第一次（多次）调用中返回 false。这是一个有意的延迟检测设计，在吞吐量和第一时间检测之间取得平衡。
...
└─ HotKeyCache.getWithSoftExpire(key, reader, ...)
      ...
      │                   │    ├─ existing.dataVersion > refreshStartDataVersion → 丢弃（有更新的写入）
      │                   │    ├─ 否则 → 更新 value + softTtl；保留 keyState, decisionVersion, hardTtls
      │                   │    └─ existing 已驱逐 → 降级为 keyState=NORMAL
...
└─ (topKAlgorithm != null)
├─ yes → topKAlgorithm.expelled() [被挤出的热点 key 队列]
└─ no → 空 LinkedBlockingQueue

HotKey.returnLocalTotalDataStreams()
└─ (topKAlgorithm != null)
├─ yes → topKAlgorithm.total() [累计访问量]
└─ no → 0L
...
```
                          App 实例（每次 get/getWithSoftExpire）
 ┌──────────────────────────────────────────────────────────────────────────────────┐
 │  record(key) → HotKeyReporter                                                    │
 │    └─ Caffeine<key, LongAdder> 计数器（30s 过期，100k 上限）                      │
 │                                                                                  │
 │  [每次 reportIntervalMs] flush()                                                 │
 │    ├─ RingManager.routeNode(key, healthView) → 一致性哈希分片目标                 │
 │    ├─ ReportDispatcher（有界队列，消费者线程池）                                   │
 ...
 │      └─ hotkey.report.{appName}.{nodeId} 队列（自动删除，每 Worker 分片）         │
 └────────────────────────────────┬─────────────────────────────────────────────────┘
                                  │
                                  ▼
 ┌──────────────────────────────────────────────────────────────────────────────────┐
 ...
 │    ├─ 陈旧检查 > 5s → 丢弃                                                        │
 │    ├─ workerTopK.add(key, count)  ──── HeavyKeeper 草图                          │
 │    ├─ SlidingWindowDetector.addCount(key, count)                                 │
 │    │    └─ AtomicLong 切片环形缓冲区 → 窗口和 >= 阈值 → isHot                     │
 │    ├─ HotKeyStateMachine.evaluate(key, isHot)                                    │
 │    │    ├─ COLD → 确认时长 → CONFIRMED_HOT                                       │
 │    │    ├─ CONFIRMED_HOT → 冷却时长 → PRE_COOLING → COLD                         │
 │    │    └─ PRE_COOLING → 重新升温 → CONFIRMED_HOT（静默，不广播）                 │
 ...
 │    ├─ 立即 ack                                                                   │
 │    ├─ 反序列化 → WorkerMessage(type, cacheKey, decisionVersion)                  │
 │    └─ 调度带随机抖动的处理（DelayUtil.floatTimeDelay）                            │
 │                                                                                  │
 │    TYPE_HOT → handleHot(wm):                                                     │
 │    ├─ VersionGuard.shouldSkipForWorker(caffeineCache, key, decisionVersion)      │
 │    │   ├─ 降级条目 → 无条件接受                                                   │
 │    │   └─ 正常条目 → 跳过如果已有 decisionVersion >= 传入值                       │
 │    ├─ redisLoader.apply(key) → 从 Redis 加载值                                   │
 │    │   └─ Redis 失败 → 回退到降级 L1 条目                                         │
 │    └─ L1 compute(): 新 CacheEntry(keyState=HOT, 延长 TTL, decisionVersion)       │
 │                                                                                  │
 │    TYPE_COOL → handleCool(wm):                                                   │
 │    ├─ compute() 内的 VersionGuard 检查                                            │
 │    └─ L1 compute(): keyState=COOL, 正常 TTL, 禁用软过期                           │
 └──────────────────────────────────────────────────────────────────────────────────┘
```
...
2. `HotKeyReporter` 在本地累计 per-key 计数（Caffeine 30s 过期，最大 100k key），周期性发布 `ReportMessage` 到 `hotkey.report.exchange` RabbitMQ 交换机，通过一致性哈希环按 `nodeId` 路由
3. Worker 节点的 `ReportConsumer` 接收报告，将计数投喂到 `workerTopK.add()`，丢弃超过 5s 的陈旧报告
4. Worker 异步处理报告——不阻塞 App 实例

### 报告路由（一致性哈希）

报告通过一致性哈希环（`RingManager`）将每个 cache key 映射到 Worker `nodeId`。哈希环在每次 flush 周期中根据心跳检测到的存活 Worker 节点集自动调整。每个 Worker 使用自动生成的 `nodeId`（通过 `InstanceIdGenerator`）声明独立的队列（`hotkey.report.{appName}.{nodeId}`），绑定到报告交换机，从而实现无需静态分片配置的自动水平扩展。
...
状态机参数（`confirmCount`、`coolCount`、`preCoolGraceCount`）可在不重启 Worker 的情况下运行时调整。当 Worker 模式激活且 `spring-boot-starter-web` 存在时，`/actuator/hotkey/worker/state` 会暴露一个 REST 端点（`StateMachineEndpoint`）。
...
通过 POST 更改 Worker 状态机参数后，新值通过基于心跳的 AMQP 机制**广播到所有对等 Worker**：

1. **专用心跳交换机** — 使用 TopicExchange（`hotkey.heartbeat.exchange`）专用于结构化心跳，与 HOT/COOL 决策流量（使用 `hotkey.broadcast.exchange`）完全隔离。每个 Worker 的 `workerConfigQueue` 使用路由键 `heartbeat.*` 绑定到此交换机。
2. **每节点自动删除队列** — 每个 Worker 声明自己的 `hotkey.worker.config.<nodeId>` 队列并绑定到心跳交换机。Worker 断开时，其队列自动删除。
3. **结构化心跳字段** — `WorkerHeartbeatProducer` 每 `ping-interval-ms`（默认为 1 秒）发送心跳，包含：`epoch`、`configTimestamp`、`decisionVersionHwm`、`confirmCount`、`coolCount`、`preCoolGraceCount`、`loadFactor`、`readyToServe` 和 `configFingerprint`。
4. **WorkerConfigNegotiator** — 每个 Worker 上的 `@RabbitListener` 接收对等方心跳。如果接收到的 `configTimestamp` 严格更新于本地时间戳，则更新自己的 `HotKeyStateMachine` 字段。
5. **启动补偿** — 启动时，每个 Worker 最多等待 3 秒以接收第一个对等方心跳。若无心跳到达，则记录 WARN 并继续使用 `WorkerProperties` 默认值。
...
> [!NOTE]
>
> - `configTimestampCounter` 从 `0` 开始——首次远程心跳的 `configTimestamp > 0` 始终被应用。
> - 没有单独的"配置变更"AMQP 消息——变更在下一个周期心跳内扩散（≤1s 延迟）。
> - 更改 `WorkerProperties` 中的 `confirmDurationMs`/`coolDurationMs`/`preCoolGraceMs` 不会更新运行时的 `confirmCount`/`coolCount`/`preCoolGraceCount`。POST 端点是启动后更改运行时值的唯一方式。如有需要，可考虑添加从 `WorkerProperties` 初始化运行时计数的启动钩子。
> - `workerConfigQueue` 绑定到专用心跳交换机（`hotkey.heartbeat.exchange`，路由键 `heartbeat.*`），而非广播交换机。这使配置八卦与 HOT/COOL 决策流量隔离。
...
### HeavyKeeper addDirect — 二项采样 (O(1) 衰减)

```
addDirect(key, increment)
│
├─ fingerprint = Murmur3_32(key)                          [Guava, 固定种子]
│
├─ 每行 depth i:
│    │  bucketIndex = Math.floorMod(hash ^ (i * golden), width)
│    │  lock = lockStripes[index & lockMask]              [256 路条纹锁]
│    │  synchronized (lock) {
│    │
│    │    if counts[index] == 0:                           [空槽位]
│    │      → fingerprints[index] = itemFingerprint
│    │      → counts[index] = increment
│    │
│    │    elif fingerprints[index] == itemFingerprint:     [相同 key，命中]
│    │      → counts[index] += increment
│    │
│    │    else:                                             [碰撞]
│    │      decayProb = lookupTable[counts[index]]          [预计算 pow(decay, count)]
│    │      decays = sampleBinomial(increment, decayProb)   [O(1) → 4 种情况]
│    │        │  n≤10:     逐次循环                           [极小，仍 O(1)]
│    │        │  np≥10:    正态近似 + 修正                    [O(1)]
│    │        │  nq≥10:    正态近似                           [O(1)]
│    │        │  其他:      逐次循环                           [有界]
│    │      if decays ≥ counts[index]:
│    │        → 抢占：fingerprints[index] = fp, count = increment
│    │      else:
│    │        → counts[index] -= decays
│    │    }
│
...
└─ maxCount ≥ minCount?
     ...
     │     ├─ key 在堆中？→ 更新计数
     │     └─ key 不在堆中？→ 插入；堆满时淘汰最低
     └─ 否 → 返回 AddResult(expelledKey=null, isHot=false)
```

### BBR + CPU Reporter 背压流程
...
├─ bbrRateLimiter != null?
│    └─ tryAcquire() → BBR 融合决策：
│         ├─ CPU 负载 (EMA, decay=0.95, 500ms 轮询) < cpuThreshold (80%)?
│         │    └─ 是（宽松模式）：
│         │         ├─ 并发 ≤ 最大并发？→ 允许
│         │         └─ 冷却中？        → 允许
│         │              └─ 其他        → 丢弃
...
│                   └─ 其他            → 丢弃
│
├─ 允许 → 清空 Caffeine 计数器，按目标分片，入队
│         └─ BbrRateLimiter.onEnqueue(batchTimestamp)
│              └─ 发布成功后 → onSuccess(RTT)
│                   ├─ 滑动窗口 (10s, 100 buckets)：maxPass + minRtt
│                   └─ 最大并发 = maxPass * minRtt (Little's Law)
│
└─ 丢弃 → 计数器保留在 Caffeine 中（下次 flush 重试）
         └─ bbrRateLimiter.onDrop()
              └─ 进入冷却 (cooldownMs = 1000ms，拒绝所有请求)
```
