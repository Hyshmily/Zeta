...
| 场景                             | P50           | P95       | P99       |
| -------------------------------- | ------------- | --------- | --------- |
| Worker 决策（未开启状态机）      | **51.64 ms**  | 97.75 ms  | 104.16 ms |
| 状态机流水线（开启状态机）       | **1,983 ms**  | 2,015 ms  | 2,015 ms  |
| 全链路（未开启状态机）           | **155.81 ms** | 202.14 ms | 212.69 ms |
| 全链路 + 状态机（20 个确认窗口） | **2,038 ms**  | 2,055 ms  | 2,055 ms  |

- **未开启状态机**（Phase 7）：滑动窗口检测后直接广播 HOT。P50=51.6ms 含 `warmupJitterMs=100ms` + AMQP + L1 轮询。
- **开启状态机**（Phase 8）：约 2.0s 由确认窗口主导（20 × 100ms = 2s），确认后走相同 AMQP 路径（约 52ms）。参数可调。
- **全链路未开启状态机**（Phase 9A）：完整端到端路径——本地 Caffeine 未命中 → 上报聚合（100ms 批次）→ AMQP 上报投递 → SlidingWindowDetector → AMQP 决策广播 → L1 提升。比 Phase 7 多约 104ms，主要来自上报批处理 + 两段 AMQP 跳转。
- **全链路 + 状态机**（Phase 9B）：同上但 Worker 端需要 20 个确认窗口。总延迟由 2s 确认底线主导。
