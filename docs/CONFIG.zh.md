...
| `computeIfAbsent(key, reader, hardTtlMs, softTtlMs, allowReport)`                | 同上，带显式硬/软 TTL 覆盖和上报控制                                                                                                                                                            |
...
| `computeIfAbsentWithSoftExpire(key, reader, hardTtlMs, softTtlMs, nullCaching, StalePolicy, allowReport)` | 全参数变体，控制空值缓存、陈旧策略和上报                                                                                                                                                         |
| `computeIfAbsentWithSoftExpire(key, reader, CachePolicy, allowReport)`           | 同上，使用显式 CachePolicy 对象（TTL、空值缓存、陈旧策略延迟求值）                                                                                                                            |
...
| `get(key, reader, CachePolicy)`                                                  | 带解析后的 per-invocation CachePolicy 读取（延迟 TTL 求值）                                                                                                                                     |
| `get(key, reader, CachePolicy, boolean)`                                         | 同上，带显式上报控制                                                                                                                                                                           |
...
| `getWithSoftExpire(key, reader, boolean)`                                        | 同上，带显式上报控制                                                                                                                                                                           |
| `getWithSoftExpire(key, reader, StalePolicy)`                                    | 同上，带显式陈旧策略                                                                                                                                                                           |
| `getWithSoftExpire(key, reader, hardTtlMs, softTtlMs, boolean)`                  | 同上，带显式硬/软 TTL 覆盖和上报控制                                                                                                                                                            |
| `getWithSoftExpire(key, reader, hardTtlMs, softTtlMs, StalePolicy)`              | 同上，带显式硬/软 TTL 覆盖和陈旧策略                                                                                                                                                            |
| `getWithSoftExpire(key, reader, hardTtlMs, softTtlMs, StalePolicy, boolean)`     | 全参数变体：TTL 覆盖、陈旧策略、上报控制                                                                                                                                                         |
| `getWithSoftExpire(key, reader, CachePolicy)`                                    | 带显式 CachePolicy 的软过期读取（延迟 TTL 求值）                                                                                                                                                |
| `getWithSoftExpire(key, reader, CachePolicy, boolean)`                           | 同上，带显式上报控制                                                                                                                                                                           |
...
| `putThrough(key, value, writer, boolean)`                                        | 同上，带显式广播控制                                                                                                                                                                          |
| `putThrough(key, value, writer, hardTtlMs)`                                      | 同上，带显式硬 TTL 覆盖                                                                                                                                                                        |
| `putThrough(key, value, writer, hardTtlMs, boolean)`                             | 同上，带显式硬 TTL 覆盖和广播控制                                                                                                                                                               |
