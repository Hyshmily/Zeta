...
| `computeIfAbsent(key, CachePolicy)`                                              | 使用显式 CachePolicy 的 compute-if-absent（携带 reader、TTL、空值缓存、陈旧策略和上报控制标志）                                                                                                |
...
| `getWithSoftExpire(key, CachePolicy)`                                            | 带显式 CachePolicy 的软过期读取（延迟 TTL 求值）                                                                                                                                                |
