...
**HotKey 通过 Redis 和 RabbitMQ 将业务代码与整个分布式协调基础设施完全解耦。** 你的应用只需调用 `hotKey.get(key, reader)` —— 本地 TopK 探测、异步上报 Worker、全集群 HOT/COOL 决策广播、跨实例缓存失效、stale-while-revalidate 异步刷新全部在后台透明完成。

HotKey 致力于以极低的成本解决集群维度对任意突发性的、无法预先感知的热点数据分布式一致性缓存问题：
