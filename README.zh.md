...
> [!NOTE]
> `@HotKey(operation=READ)` 将原始方法调用包装为 L2 supplier——方法体同时充当缓存读取和数据库回退。这对读密集型端点很方便，但会绕过 `peek()` 语义。`softExpire=true`（默认）启用 stale-while-revalidate：过期值立即返回，后台线程异步刷新。`softExpire=false` 时，缓存未命中总是同步调用方法。
