...
HotKey 是一个[高性能](docs/HotKey_Benchmark_Report.zh.md)、低成本、轻量级的全链路热 Key 治理 Spring Boot Starter，集**缓存读写（get/put）、热点自动检测、多级缓存预热、跨实例广播同步、AOP 注解拦截、黑白名单过滤**于一体。
...
- **请求合并** — L1 未命中时同 key 并发请求共享同一 L2 读，通过专门的 `SingleFlight` bean
  ...
  > [!NOTE]
  > 确保 `hotkey.local.inflight-ttl-seconds` 大于最慢的 L2 响应耗时，否则缓存条目可能在 Future 完成前过期，导致重复的 L2 读。
...
- **Worker 模式** — 专用集群维度热点检测节点；基于滑动窗口 + 状态机管道实现跨实例共识；详见 [Worker 模式](#worker-模式)
...
- **一致性哈希** — 基于 murmur3_32 的一致性哈希环替代静态 `shard-index` 映射；Worker 通过心跳自动注册，支持弹性扩缩容而无需完全 key 重映射（通过 `hotkey.local.consistent-hashing.enabled` 开启）
- **Micrometer 指标** — 18+ 数值型 Gauge/Counter（Caffeine 缓存、TopK、inflight、reporter、版本降级、Worker 健康），检测到 `micrometer-core` 时自动注册 `MeterBinder` beans
- **原始 L1 缓存访问** — `HotKey.getLocalCache()` 暴露底层 `Cache<String, Object>`，用于 Caffeine 特定操作（`asMap`、`policy`、`cleanUp`），附有绕过编排层的警告说明
...
> **极限性能**
>
> 将部分参数设为极限值可将全链路延迟压缩至 **~9ms**（P50），但作者**不推荐**生产环境使用。
>
> HotKey 的默认参数偏向保守，框架应该优先保证分布式集群的可靠性而非极致延迟：
>
> **状态机通过”每个 Key 每个生命周期只广播一次”**，从根本上消除了冗余广播导致的客户端 CPU 过载和自造拥塞。极限测试中，SM 0 确认路径仅产生 10 条广播，而无状态机路径产生了大量冗余广播，延迟因此大幅放大。
>
> **默认的 2 秒确认窗口**不是反应迟钝，而是用这 2 秒的连续观察，换取几乎零误报的全局决策。在这 2 秒内，本地 HeavyKeeper 已在纳秒级完成保护，用户请求没有经历任何阻塞。
>
> **预热抖动和批处理间隔**是分布式系统中经典的“防惊群”和“平滑负载”手段，牺牲几十毫秒的延迟，换来的是全集群的稳定。
>
> 极限参数的意义在于**展示此框架全链路的性能上限**（~9ms），并作为极端场景的调优参考。除非你完全理解并愿意承担误报率升高、统计精度下降、客户端 CPU 过载等代价，否则请使用默认值或经过严格测试的参数。
...
每跳延迟（[延迟分布图](docs/img/latency_distribution_heatmap.png)）：

| 路径                                | 说明                                             | P50            | P95            | P99            |
| ----------------------------------- | ------------------------------------------------ | -------------- | -------------- | -------------- |
| L1 命中（Caffeine 查找）            | 纯内存查找，无网络 I/O                           | **0.001 ms**   | 0.004 ms       | 0.011 ms       |
| L1 未命中 → Redis → L1              | Redis GET RTT + SingleFlight 去重 + L1 回填      | 0.47 ms        | 0.87 ms        | 1.65 ms        |
| AMQP 发布                           | RabbitMQ 通道写入（内存到内存）                  | 0.02 ms        | 0.12 ms        | 0.16 ms        |
| AMQP 端到端投递                     | 发布 + 代理路由 + 消费者投递                     | 0.07 ms        | 0.25 ms        | 0.38 ms        |
| Redis GET 往返                      | 纯网络往返                                       | 0.44 ms        | 0.91 ms        | 1.91 ms        |
| Worker 决策流水线                   | 滑动窗口检测 → AMQP 广播，含 jitter+AMQP+L1 轮询 | **56.38 ms**   | 99.21 ms       | 103.56 ms      |
| 状态机流水线（3 确认窗）            | 3 确认窗(300ms) + AMQP 决策广播 + L1 提升        | **246.46 ms**  | 295.00 ms ^^   | 295.00 ms ^^   |
| **全链路（SM 3 确认窗）**           | **分步构成见下方**                               | **298.19 ms**  | **351.50 ms ^^**  | **351.50 ms ^^**  |
| ┣ L1 未命中 → Redis → L1            | 与 Phase 6 同路径                                | 0.47 ms        | 0.87 ms        | 1.65 ms        |
...
| ┣ SM 确认流水线（3 窗口）           | 3 确认窗 + AMQP 广播 + WorkerListener + L1 提升 | ~246 ms        | 295.00 ms      | 295.00 ms      |
| ┗ 余量（调度 + 轮询 + 噪音）        | isLocalHotKey() 轮询 + 调度方差                   | ~1 ms          | — ^            | — ^            |

> ^ P95/P99 不可加——分步的尾延迟并不叠加为合成分步的尾延迟。只有独立可测的分步（L1 未命中 → Redis、SM 流水线）展示真实的 P95/P99。P50 之和（~298ms）近似匹配全链路 P50（298.19ms），因为均值可加。全链路 P50 **298ms** 距理论 300ms 确认底线仅 **0.6%**。
> ^^ 全链路 SM 3 确认和 SM 流水线阶段 P95=P99=Max，因为每项只用 **10 个键**（每键一次测量）。10 个样本点时 P95 = 第 9.5 个 → 第 10 个值 = Max，P99 = 第 9.9 个 → 第 10 个值 = Max。百分位数等于最大值，非测量异常。
...
所有场景都使用状态机 — 区别在于 `confirm-duration-ms=300`（默认 3 个确认窗口）和`0`（首个热窗口即广播）。状态机控制广播量：每个键在整个生命周期中仅广播一次，不受确认窗口数量的影响。所有场景均受限于 Redis GET RTT（~0.5 ms P50）和 AMQP 投递（~2.5 ms P50）。

**代价：**

- 极限配置下，上报频次从约 10 批/秒飙升至约 1000 批/秒。RabbitMQ 服务端本身可以承受此负载，但**如果没有状态机的广播压缩，Worker 会将每次滑动窗口判定都转化为一次广播**，导致客户端消费端收到海量冗余消息，CPU 消耗剧增。**状态机通过”每个 Key 每个生命周期只广播一次”彻底消除了这个风险**——在 SM 0 确认的极限测试中，10 个 Key 只产生了 10 条广播，而相同条件下无状态机路径产生了大量冗余广播，延迟因此大幅放大（68.95ms vs 9.23ms）。
...
hotkey:
  ...
  # 一致性哈希（动态 Worker 路由，需启用 worker-listener）
  # local:
  #   consistent-hashing:
  #     enabled: true
```
...
hotkey:
  worker:
    ...
    routing:
      ...
      node-id: ""   # 显式 nodeId（一致性哈希使用，"" = 自动从 InstanceIdGenerator 生成）
...
hotkey:
  local:
    ...
    # ——— 一致性哈希替代 shard-index 路由 ———
    consistent-hashing:
      enabled: false                        # 启用动态 Worker 路由
      virtual-nodes: 150                    # 每个物理 Worker 的虚拟节点数
...
> 完整文档：[`docs/ANNOTATION.zh.md`](docs/ANNOTATION.zh.md) —— 包含配套注解（`@Fallback`、`@Intercept`、`@HotKeyCacheTTL`）、优先级链、SpEL condition/unless 支持、返回类型处理和完整示例。
...
需要在 classpath 中包含 `spring-boot-starter-aop`（提供 AspectJ）。SpEL 参数名解析需要 `-parameters` 编译标志（HotKey parent POM 默认启用）。
...
hotKey.get(key, supplier)
  ...
  └─ HotKey 自身异常 → 异常 → 降级（@Fallback 存在时）→ 调用方
```
