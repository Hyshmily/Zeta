...
所有选项见[配置](#配置)，完整属性参考见 [README.zh.CONFIG.md](README.zh.CONFIG.md)。
...
快速入门配置见[### 2. 配置](#2-配置)。完整属性列表见 [README.zh.CONFIG.md](README.zh.CONFIG.md)。Worker 模式见 [README.zh.WORKER.md](README.zh.WORKER.md)。
...
启用 `hotkey.worker.enabled=true`。两种部署模式——见 [Worker 模式](#worker-模式)和 [README.zh.WORKER.md](README.zh.WORKER.md)。
...
- **`TYPE_REFRESH`** — 带版本号的失效。对端通过 `CacheSyncListener.handleRefresh()` 从 Redis 重新加载，根据 {@code dataVersion} 跳过旧更新。4 种情况的比较（正常-正常、正常-降级、降级-正常、降级-降级）保证正常的（Redis INCR）dataVersion 始终优先于降级的（节点本地）版本。
- **`TYPE_INVALIDATE`** — 批量失效（`invalidateAll`）。对端立即从 L1 移除该 key，不重新加载。

### Worker Listener

接收专用 Worker 节点发出的 HOT/COOL 决策，启用 `hotkey.worker-listener.enabled=true`。
