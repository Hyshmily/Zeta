...
HotKey — HeavyKeeper Top-K 热点检测 + 多级缓存自动预热 + 分布式缓存同步 + 集群 Worker 模式 Spring Boot Starter
...
通过 [HeavyKeeper](https://github.com/go-kratos/aegis)（Count-Min Sketch 变体）概率检测访问频率。**读路径**上，只有进入 Top-K 集合的 key 才会被自动提升到本地 Caffeine L1，并通过可选的 RabbitMQ 跨实例同步。热点 key 和普通 key 使用**差异化 TTL**——热点 key 缓存更久（默认 1h），普通 key 更快过期（5min）。**写路径**上，`putThrough` 无论 key 是否为热点，都直接写入 L1 并广播——调用方显式拥有写入权。非热点 key 的读取仍然通过调用方提供的 `Supplier<T>` 获取并返回值——只是在 L1 中不保留长 TTL。
...
```yaml
hotkey:
  soft-ttl-ms: 5000 # 覆盖普通 key 的软 TTL（0 = 回退到默认值）
  hot-soft-ttl-ms: 300000 # 覆盖热点 key 的软 TTL（0 = 回退到默认值）
  refresh-max-pools: 50 # 限制异步刷新并发数
```
...
如需跨多实例的集群维度热点检测，详见 [README.zh.WORKER.md](README.zh.WORKER.md)。
...
> **注意：** `invalidate()` 通过 Redis `INCR` 生成单调版本号，以 `TYPE_REFRESH` 广播——对端通过 `CacheSyncListener` 从 Redis 重新加载，跳过旧版本。`invalidateAll()` **不调用** `INCR`——它以 `TYPE_INVALIDATE` 广播，版本 `0L`，所有对端无条件从 L1 移除该 key。
...
| ----------------- | ---- | --------- |
| `hard-ttl-ms`     | 普通 | 0（默认） |
| `hot-hard-ttl-ms` | 热点 | 0（默认） |
| `soft-ttl-ms`     | 普通 | 0（默认） |
| `hot-soft-ttl-ms` | 热点 | 0（默认） |
...
| 全局 `local-cache-ttl-minutes`                         | 全局写入 TTL（被差异化 TTL 补充）                               |
| 全局 `local-cache-access-ttl-minutes`                  | 基于访问的硬 TTL（每次读取重置），作为写入 TTL 的补充           |
...
详见 [README.zh.WORKER.md](README.zh.WORKER.md)。
...
| 模式        | `worker.enabled` | `worker.exclusive-mode` | 激活的 Bean                         |
| ----------- | ---------------- | ----------------------- | ----------------------------------- |
| App-only    | `false`（默认）  | —                       | `HotKeyCache`、TopK、actuator、sync |
| Worker-only | `true`           | `true`（默认）          | 仅 Worker（无缓存）                 |
| Coexistence | `true`           | `false`                 | 所有 Bean（App + Worker 同 JVM）    |

完整文档（Worker 搭建、状态机、滑动窗口、配置）见 [README.zh.WORKER.md](README.zh.WORKER.md)。
...
详见 [README.ARCH.md](README.ARCH.md)（英文）和 [README.zh.ARCH.md](README.zh.ARCH.md)（中文）。

## 配置参考

详见 [README.CONFIG.md](README.CONFIG.md)（英文）和 [README.zh.CONFIG.md](README.zh.CONFIG.md)（中文）。
