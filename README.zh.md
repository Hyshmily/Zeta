...
HotKey 是一款强大的[高性能](docs/HotKey_Benchmark_Report.zh.md)、低成本、轻量级分布式缓存框架，集**缓存读写（get/put）、热点自动检测、多级缓存预热、跨实例广播同步、AOP 注解拦截、黑白名单过滤**于一体。
...
- **注解驱动缓存** — `@HotKey` + `@HotKeyCacheTTL` + `@Intercept` + `@Fallback` 零侵入接入；SpEL key 解析、`softExpire` 开关、TTL 继承
