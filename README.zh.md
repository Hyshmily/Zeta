...
- 潮汐节奏是自适应的：`TidePacer`（热点先导）将交付积压折叠成快攻/慢放的平滑节奏——突发潮汐把周期缩短至 50ms，空闲潮汐则加倍拉长；`MoonsTidalForce`（月球潮汐引力修正）在续热键受阻时把提升门槛降到直方图边界直接接纳，而非棘轮式上调。两个按流量分档的 regime 开关扩展了它（ADR-0047）：低流量 regime（每 tide 少于 50 次计数、热集为空）把门槛降到 1，任意 key 都走热路径；抬高的旧门槛挡住整个新分布且流量很高时，一个 tide 内塌缩归位（洪水塌缩），不必等缓慢的衰减式恢复。提升边界本身按密度细化：边界桶溢出剩余热槽时，线性子直方图把阈值落到真实的第 k 大计数，且续热成员先于新来者被重新提升——平坦分布下热集保持稳定，而非每潮汐轮换随机样本
