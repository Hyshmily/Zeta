...
- 潮汐节奏是自适应的：`TidePacer` 在突发时把交付周期缩短至 50ms、空闲时加倍拉长；`MoonsTidalForce` 提升门槛调节器在续热键受阻时把门槛降到边界直接接纳（绝不棘轮上调），并带两个按流量分档的 regime 开关（低流量归 1、洪水时旧门槛一个 tide 塌缩归位）与有界、带退避定价的探针 walk（ADR-0045/0051/0052/0053）。提升边界本身是精确的（ADR-0054）：边界值是该周期第 k 大计数的精确值，由 quickselect 在 log2 定位的桶内选出，并列时续热成员先于新来者重新提升——平坦分布下热集保持稳定；稳定负载下选择复用上一周期边界作为扫描过滤器，选择规模从整个桶降为约 `hotLimit` 个键（ADR-0057）
