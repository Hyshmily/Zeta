...
```
          ...
          │              │              │              │
L1 Hit    █┤  0.01ms ← (Caffeine 直接命中, 即时返回)

L1 Miss   ██┤  +~10ms ← SingleFlight(3s超时) + L2 Redis/数据库 回源

上报批处理 ███████████┤  +~25ms(P50) ← report-interval-ms=50 满触发 flush

↑AMQP发布  ██┤  +~5ms ← 网络传输 + RabbitMQ入队

Worker消费 ██┤  +~5ms ← 出队 + 反序列化

滑窗检测   ██████████████████████████████┤  +~100ms (热点 ~100ms 内可辨识)

状态机确认 ██████████████████████████████┤  +~100ms (2×confirm窗, 每个50ms)

↓决策广播  ██┤  +~5ms (AMQP fanout 到所有 App 实例)

预热抖动   ███┤  +~25ms(P50) ← warmupJitterMs=50

L1 缓存   █┤ +~5ms ← (查询Redis,Caffeine缓存)
```
