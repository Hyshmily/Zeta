...
- 上报路径内置 BBR 拥塞控制算法，根据 CPU 负载自动降速，防止突发流量打垮通道。 Worker 集群聚合所有应用实例的访问报告，经过滑动窗口频率分析与贝叶斯置信度状态机仲裁，将 HOT/COOL 决策广播回每个实例。每个缓存条目（CacheEntry）携带 dataVersion 与 decisionVersion 两个正交版本号，KeyState 标记其生命周期状态（HOT/COOL/NORMAL）。 Worker 决策通过单调递增的 decisionVersion 覆盖本地提升结果，确保集群一致性。
