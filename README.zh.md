...
> **极限性能**
>
> 将部分参数设为极限值可将全链路延迟压缩至 **~7.5ms**（P50），但作者**不推荐**生产环境使用。
>
> HotKey 的默认参数偏向保守，框架应该优先保证分布式集群的可靠性而非极致延迟：
>
> **状态机通过“每个 Key 每个生命周期只广播一次”**，从根本上消除了冗余广播导致的客户端 CPU 过载和自造拥塞。极限测试中，SM 0 确认路径仅产生 10 条广播，而无状态机路径产生了 86 条，延迟因此放大了约 9 倍（65.88ms -> 7.54ms）。
>
...
>
> 极限参数的意义在于**展示此框架全链路的性能上限**（~7.5ms），并作为极端场景的调优参考。除非你完全理解并愿意承担误报率升高、统计精度下降、客户端 CPU 过载等代价，否则请使用默认值或经过严格测试的参数。
...
| Worker 决策流水线                   | 滑动窗口检测 → AMQP 广播，含 jitter+AMQP+L1 轮询 | **51.64 ms** | 97.75 ms     | 104.16 ms    |
| 状态机流水线（开状态机，20 确认窗） | 20 确认窗(2s) + AMQP 决策广播 + L1 提升          | **1,983 ms** | 2,015 ms     | 2,015 ms     |
| **全链路（SM 20 确认窗）**          | **分步构成见下方**                               | **2,038 ms** | **2,055 ms** | **2,055 ms** |
...
| ┣ SM 确认流水线（20 窗口）          | 20 连续热窗口 × 100ms 时间片                     | ~2,000 ms    | —            | —            |
...
| 场景              | 默认(20确认窗) P50 | 极限(0确认窗) P50 | 极限 P95 | 极限 P99 |
| ----------------- | ------------------ | ----------------- | -------- | -------- |
| Worker 决策流水线 | 63.78 ms           | **2.35 ms**       | 3.77 ms  | 5.16 ms  |
| SM 确认流水线     | 2,000.42 ms        | **6.80 ms**       | 8.03 ms  | 8.03 ms  |
| 全链路            | 2,024.38 ms        | **7.54 ms**       | 8.56 ms  | 8.56 ms  |

所有场景都使用状态机 — 区别在于 `confirm-duration-ms=2000`（默认 20 个确认窗口）和`0`（首个热窗口即广播）。状态机控制广播量：每个键在整个生命周期中仅广播一次，不受确认窗口数量的影响。所有场景均受限于 Redis GET RTT（~0.5 ms P50）和 AMQP 投递（~2.5 ms P50）。

**代价：**

- 极限配置下，上报频次从约 10 批/秒飙升至约 1000 批/秒。RabbitMQ 服务端本身可以承受此负载，但**如果没有状态机的广播压缩，Worker 会将每次滑动窗口判定都转化为一次广播**，导致客户端消费端收到海量冗余消息，CPU 消耗剧增。**状态机通过“每个 Key 每个生命周期只广播一次”彻底消除了这个风险**——在 SM 0 确认的极限测试中，10 个 Key 只产生了 10 条广播，而相同条件下无状态机路径产生了 86 条广播，延迟因此放大了约 9 倍（65.88ms vs 7.54ms）。
