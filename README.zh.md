...
> **注意：** `mutation.run()` 到 `caffeineCache.invalidate()` 之间存在约 1ms 的不一致窗口，期间并发的 `get()` 可能命中 L1 旧值。这是刻意的取舍——如果在 mutation 之前失效，`get()` 会从 Redis 读到旧值重新填入 L1，造成更严重的数据污染。该窗口宽度由一次 Redis 往返（`nextVersion` 调用）决定。
