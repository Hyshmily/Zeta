...
HotKey 是一款高可配置、高性能、低成本的轻量级分布式缓存与预热框架,HotKey 通过 Redis 和 RabbitMQ 将业务代码与整个分布式协调基础设施完全解耦。

它致力于以极低的成本解决集群维度对任意突发性的、无法预先感知的热点数据分布式一致性缓存问题：
...
- **自动 L1 压缩** — 大于 256 字节的 `String` 值在写入 Caffeine L1 缓存时自动 LZ4 压缩。压缩在 `CacheEntry` 创建层自动完成，无需手动调用 `compress`/`decompress`。当 `lz4-java` 不可用时自动降级为无操作。非 `String` 类型原样透传。
...
<details>
<summary><b>3. 多种限流算法集群保护</b></summary>
...
<details>
<summary><b>4. 多级故障保护与降级处理</b></summary>
...
- **滑动窗口 Sketch** — 每个 sketch slot 使用旋转环形缓冲区（默认 3 窗口）。持续的访问低谷会被轮转掉而非衰减当前计数，长期高热 key 保持排名，新上升 key 快速入堆
...
>
> 默认设置下全链路端对端延迟约为:**300ms (P99)**，此延迟包含 HTTP 路由、序列化、Tomcat 线程调度等开销。
> 直接 Java 调用的基准测试：
>
...
hotkey:
  local:
    ...
    # ——— L1 Caffeine 缓存 ———
    cache:
      max-size: 100000 # 最大条目数（max-weight 为 0 时生效）
      max-weight: 0 # 内存权重限制（字节），0=禁用
      max-value-size: 0 # 单值字节大小限制，0=不限
    ...
    # ——— Null 值 TTL ———
    null-value-ttl-seconds: 10 # null/缓存未命中条目的 TTL（短，避免缓存负结果过久）
    ...
    heartbeat:
      ...
      verify-max-backoff-ms: 600000 # 单 Worker 指数退避最大间隔（10min）
      min-alive-workers: 0 # 0=动态（有1个存活即健康）；设置>0则要求至少N个Worker存活
...
// J1. registerRefresh / updateRefresh — 定时后台刷新（softTtlMs = 间隔）
hotKey.registerRefresh("user:123", () -> loadUser(123), 300_000L, 60_000L);  // 每 60s
hotKey.updateRefresh("user:123", () -> loadUser(123), 300_000L, 30_000L);    // 改为 30s
hotKey.unregisterRefresh("user:123");                                         // 停止
