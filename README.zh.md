...
HotKey 是一款[高性能](docs/HotKey_Benchmark_Report.zh.md)、低成本、轻量级的分布式缓存框架，集**缓存读写（get/put）、热点自动检测、多级缓存预热、跨实例广播同步、AOP 注解拦截、黑白名单过滤**于一体。
...
> [!IMPORTANT]
> 在分布式部署情况下,除了RabbitMQ,作者强烈建议引入Redis以达到最佳效果,尽管作者在核心调用链条上做了很多降级处理。
...
Worker 模式通过专用节点提供集群维度热点检测。App 实例定期报告访问计数，Worker 运行滑动窗口+状态机管道，将 HOT/COOL 决策广播回所有实例。状态机参数（`confirmCount`、`coolCount`、`preCoolGraceCount`）可通过 `/actuator/hotkey/worker/state` 运行时调整——变更通过基于心跳的 AMQP 广播传播到所有对等 Worker。
