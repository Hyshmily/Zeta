...
- **Rebroadcast** — Periodic re-emission of a HOT decision by the Worker state machine while a key stays in `CONFIRMED_HOT`, at most once per `zeta.worker.state-machine.rebroadcast-interval-ms` (default 10s). Recovers HOT broadcasts lost in flight (fire-and-forget, ADR-0007) and caps fast-lane steady-state emission to one HOT decision per interval. The stamp (`KeyState.lastBroadcastAt`) is optimistic — set at decision time; a failed AMQP send rolls it back to 0 via `rollbackToPreviousState`, so the next evaluation retries immediately. COOL is never rebroadcast: a lost COOL fails in the lenient direction, bounded by hard TTL and stale eviction. See ADR-0024.
