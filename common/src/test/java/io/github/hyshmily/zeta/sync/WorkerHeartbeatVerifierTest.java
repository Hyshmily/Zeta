...
class WorkerHeartbeatVerifierTest {
  ...
  void shouldHandleMixedResults() {
    when(rabbitTemplate.sendAndReceive(eq(""), eq("zeta.verify.ping.w2"), any())).thenReturn(
      new Message(new byte[0], new MessageProperties())
    );
    when(rabbitTemplate.sendAndReceive(eq(""), eq("zeta.verify.ping.w3"), any())).thenReturn(null);
