...
/**
 * Tests for {@link ZetaLockAutoConfiguration}.
 */
class ZetaLockAutoConfigurationTest {

  private final ApplicationContextRunner runner = new ApplicationContextRunner().withConfiguration(
    AutoConfigurations.of(ZetaLockAutoConfiguration.class)
  );
  ...
  void configIsSkippedWhenStringRedisTemplateBeanNotAvailable() {
    new ApplicationContextRunner()
      ...
      .withConfiguration(AutoConfigurations.of(ZetaLockAutoConfiguration.class))
      .run(ctx -> {
        assertThat(ctx).doesNotHaveBean(LockProvider.class);
      });
  }
