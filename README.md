...
- The tide is self-adaptive: the `TidePacer` (hot-spot pilot) folds the delivered backlog into a fast-attack/slow-release smoothed cadence — bursts shorten the cycle to 50ms, idle stretches double it — while the `MoonsTidalForce` (tide-gravity correction) drops the promotion floor to the histogram boundary to admit blocked renewing keys instead of ratcheting it up. Two volume-gated regime switches extend it (ADR-0047): a quiet regime (volume below 50 counts/tide, empty hot set) drops the floor to 1 so any key routes hot, and a stale raised floor blocking the whole incoming distribution at high volume collapses in one tide (the flood collapse) instead of waiting for the slow decay-driven recovery. The promotion boundary itself is density-refined: when the boundary bucket overflows the remaining hot slots, a linear sub-histogram lands the threshold at the actual ~k-th largest count, and renewing members are re-promoted before newcomers — the hot set stays stable under flat distributions instead of rotating a random sample.
