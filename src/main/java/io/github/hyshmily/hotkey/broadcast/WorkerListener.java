...
import static io.github.hyshmily.hotkey.worker.WorkerMessage.TYPE_COOL;
import static io.github.hyshmily.hotkey.worker.WorkerMessage.TYPE_HOT;
...
import io.github.hyshmily.hotkey.worker.WorkerMessage;
...
public class WorkerListener {
   ...
   * ensuring we never overwrite a newer version that arrived concurrently.
   */
  private void handleHot(WorkerMessage wm) {
    // DCL first check – cheap, outside the compute lock
    if (VersionGuard.shouldSkipForWorker(caffeineCache, wm.cacheKey(), wm.version()) != null) {
    ...
    caffeineCache
      ...
      .compute(wm.cacheKey(), (key, existing) -> {
        ...
        if (VersionGuard.shouldSkipForWorker(caffeineCache, key, wm.version()) != null) {
          return existing;
        }
        ...
        return new CacheEntry(
          value,
          wm.version(),
          false, // never degraded from Worker
          expireManager.getEffectiveHotHardTtlMs(),
   ...
   * <p>
   * The existing cached value, version, and degradation flag are preserved.
   * The hard TTL is reset to the normal value and soft expiration is fully
   * disabled (both softTtlMs and softExpireAtMs set to 0) so that the entry
   * stops being proactively refreshed. The data remains in the cache and
   * will be evicted naturally by Caffeine's capacity policy or a subsequent
   * invalidation.
   */
      ...
      .compute(wm.cacheKey(), (key, existing) -> {
        ...
        if (existing instanceof CacheEntry cacheEntry) {
          ...
          return new CacheEntry(
            ...
            cacheEntry.isVersionDegraded(),
            cacheEntry.getNormalHardTtlMs(),
            expireManager.computeHardExpireAt(cacheEntry.getNormalHardTtlMs()),
            0, // disable soft TTL
            0, // clear soft expire timestamp
            KeyState.COOL,
            normalHardTtlMs,
            normalSoftTtlMs
          );
