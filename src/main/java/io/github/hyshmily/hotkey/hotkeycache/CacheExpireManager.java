...
public class CacheExpireManager {
  ...
  public void triggerBackgroundRefresh(String cacheKey, Supplier<?> reader, long softTtlMs) {
    ...
    long refreshStartVersion = Optional.ofNullable(caffeineCache.getIfPresent(cacheKey))
      .filter(CacheEntry.class::isInstance)
      .map(CacheEntry.class::cast)
      .map(CacheEntry::getVersion)
          ...
          caffeineCache
            ...
            .compute(cacheKey, (_, existing) ->
              Optional.ofNullable(existing)
                ...
                .map(CacheEntry.class::cast)
                .map(cacheEntry -> {
                  if (cacheEntry.getVersion() > refreshStartVersion) {
                  ...
                  return new CacheEntry(
                    ...
                    cacheEntry.getHardTtlMs(),
                    cacheEntry.getHardExpireAtMs(),
                    softTtlMs,
