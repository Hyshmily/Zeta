 ...
 * Worker is active in exclusive mode ({@code hotkey.worker.enabled=true}
 * and {@code hotkey.worker.exclusive-mode=true}).  It runs in all other
 * modes (pure App, coexistence, or Worker disabled).
 */
@Slf4j
@AutoConfiguration
@ConditionalOnExpression(
  "'${hotkey.worker.enabled:false}' != 'true' or '${hotkey.worker.exclusive-mode:true}' == 'false'"
)
