 ...
 *   <li><b>Coexistence mode</b> ({@code hotkey.worker.enabled=true, exclusive-mode=false}):
  ...
  private void requireCache() {
    if (hotKeyCache == null) {
      throw new UnsupportedOperationException(
        "HotKey cache is not available in the current mode (Worker-only). "
          + "Set hotkey.worker.exclusive-mode=false to enable coexistence."
      );
    }
  }
}
