...
public class WorkerAutoConfiguration {
  ...
  void validateWindowConfig() {
    if (properties.getWindowDurationMs() % properties.getWindowSlices() != 0) {
      log.warn(
        "windowDurationMs ({}) is not evenly divisible by windowSlices ({}). "
          + "This introduces rounding inaccuracies in window calculations.",
        properties.getWindowDurationMs(), properties.getWindowSlices()
      );
    }
  }
  ...
  public SlidingWindowDetector slidingWindowDetector(WorkerProperties properties) {
    long threshold = properties.getHotThreshold();
    if (threshold <= 0) {
      threshold = Long.MAX_VALUE;
    }
    return new SlidingWindowDetector(properties.getWindowDurationMs(), properties.getWindowSlices(), threshold);
  }
  ...
  public TopK workerTopK(WorkerProperties properties) {
    return new HeavyKeeper(
      properties.getWorkerTopK(),
      properties.getWorkerWidth(),
      properties.getWorkerDepth(),
      properties.getWorkerDecay(),
      properties.getWorkerMinCount()
    );
  }
  ...
  public TopKValidator topKValidator(
    ...
    return new TopKValidator(
      workerTopK,
      broadcaster,
      properties.getTopkPreWarmCount(),
      properties.getTopkPreWarmMinAppearances()
    );
  }
  ...
  public WorkerBroadcaster workerBroadcaster(RabbitTemplate rabbitTemplate, WorkerProperties properties) {
    return new WorkerBroadcaster(rabbitTemplate, properties.getBroadcastExchange(), properties.getAppName());
  }
  ...
  public Queue reportQueue(WorkerProperties properties) {
    String queueName = HotKeyConstants.QUEUE_PREFIX_REPORT + properties.getAppName() + "." + properties.getShardIndex();
  ...
  public Binding reportBinding(Queue reportQueue, DirectExchange reportExchange, WorkerProperties properties) {
    String routingKey = HotKeyConstants.ROUTING_KEY_REPORT + properties.getAppName() + "." + properties.getShardIndex();
  ...
  public static class EvictStaleTask {
    ...
    @Scheduled(fixedDelayString = "${hotkey.worker.cool-duration-ms:15000}")
    public void evictStale() {
      long staleAfterMs = properties.getCoolDurationMs() * 2;
   ...
   *
   * <p>Interval is controlled by {@code hotkey.worker.topk-validate-interval-ms}
   * (default 60 seconds).
   */
  @Bean
  ...
  public static class TopKValidationTask {
    ...
    @Scheduled(fixedDelayString = "${hotkey.worker.topk-validate-interval-ms:60000}")
