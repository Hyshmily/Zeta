 ...
 */
package io.github.hyshmily.zeta.worker;
...
class WorkerApplicationTest {
   ...
   * mocked RabbitMQ/Redis infrastructure when {@code zeta.worker.enabled=true}.
   */
  @Test
  ...
  /**
   * Verifies that the {@link Zeta} facade bean is also created by
   * {@link ZetaFacadeAutoConfiguration}
  ...
  void hotKeyFacadeBeanIsCreated() {
    assertThat(applicationContext.getBean(Zeta.class)).isNotNull();
  }
   ...
   * {@code io.github.hyshmily.zeta.worker.config.WorkerAutoConfigurationTest.MinimalMockConfiguration}.
   */
  @TestConfiguration
