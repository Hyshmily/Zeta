...
| 属性                                                      | 默认值  | 说明                                |
| --------------------------------------------------------- | ------- | ----------------------------------- |
| `hotkey.worker.threshold.hot-threshold`                   | `-1`    | 绝对热阈值（设为 -1 启用比率模式）  |
...
| 属性                                                            | 默认值  | 说明                                    |
| --------------------------------------------------------------- | ------- | --------------------------------------- |
| `hotkey.worker.dynamic-threshold.recalculate-interval-ms`       | `60000` | 重新计算间隔（60s）                     |
| `hotkey.worker.dynamic-threshold.qps-change-tolerance`          | `0.5`   | 触发阈值更新所需的 QPS 变化幅度（±50%） |
...
**Worker-only** 模式下，`HotKey.isHotKey()` / `get()` / `putThrough()` 抛出 `UnsupportedOperationException`——这些操作需要应用端缓存。Worker-TopK 查询（`returnWorkerHotKeys()`）仍可用。
