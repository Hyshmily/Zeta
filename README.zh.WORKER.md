...
1. 每个 App 实例运行 `HotKeyReporter`，每 `reportIntervalMs`（默认 100ms）将本地 HeavyKeeper 数据批量组成 `ReportMessage` 记录
2. 报告发布到 `hotkey.report.exchange` RabbitMQ 交换机，按 `app-name` 和 `shard-index` 路由
3. Worker 节点的 `ReportConsumer` 接收报告，将访问计数投喂到 `SlidingWindowDetector`
...
| 属性                               | 默认值 | 说明                             |
| ---------------------------------- | ------ | -------------------------------- |
| `hotkey.worker.window-duration-ms` | `1000` | 窗口总时长（1 秒）               |
| `hotkey.worker.window-slices`      | `10`   | 每个窗口的时间片数（每片 100ms） |
...
| 属性                                | 默认值  | 说明                                |
| ----------------------------------- | ------- | ----------------------------------- |
| `hotkey.worker.hot-threshold`       | `-1`    | 绝对热阈值（设为 -1 启用比率模式）  |
| `hotkey.worker.hot-threshold-ratio` | `0.01`  | 热阈值占估计全局 QPS 的比例         |
| `hotkey.worker.confirm-duration-ms` | `2000`  | 高于阈值确认 HOT 的持续时间（2s）   |
| `hotkey.worker.cool-duration-ms`    | `15000` | 低于阈值确认 COOL 的持续时间（15s） |
| `hotkey.worker.pre-cool-grace-ms`   | `5000`  | 静默重新升温的宽限期（5s）          |
...
| 属性                                    | 默认值  | 说明                                    |
| --------------------------------------- | ------- | --------------------------------------- |
| `hotkey.worker.recalculate-interval-ms` | `60000` | 重新计算间隔（60s）                     |
| `hotkey.worker.qps-change-tolerance`    | `0.5`   | 触发阈值更新所需的 QPS 变化幅度（±50%） |
...
| 属性                                          | 默认值  | 说明                          |
| --------------------------------------------- | ------- | ----------------------------- |
| `hotkey.worker.topk-validate-interval-ms`     | `60000` | 交叉验证间隔（60s）           |
| `hotkey.worker.topk-pre-warm-count`           | `5`     | 符合预预热条件的 Top-K 条目数 |
| `hotkey.worker.topk-pre-warm-min-appearances` | `2`     | 预预热所需的最小连续出现次数  |
...
| ----------- | ---------------- | ----------------------- | --------------------------------------------------------------------- |
| App-only    | `false`（默认）  | —                       | `HotKeyCache`、TopK 检测器、actuator、sync                            |
| Worker-only | `true`           | `true`（默认）          | 仅 Worker（SlidingWindow、StateMachine、ReportConsumer、Broadcaster） |
| Coexistence | `true`           | `false`                 | 所有 Bean（App + Worker）— 用于开发/测试                              |

在 **Worker-only** 模式下，调用 `HotKey.isHotKey()` / `get()` / `putThrough()` 会抛出 `UnsupportedOperationException`——这些操作需要应用端缓存。Worker-TopK 查询（`returnWorkerHotKeys()`）仍可用。
